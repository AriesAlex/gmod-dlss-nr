...
local desiredForceHardwareSkinning = PersistBinaryConVar(
    "r_forcehwskin",
    "0",
    -- Forced hardware skinning drops animated meshes in this GMod build.
    hl2RTXMounted and "0" or nil
)

-- A named timer replaces an outstanding restore when Lua is refreshed.
-- Poll in real time: zero-delay retries can expire before queued commands run.
timer.Create("RTXHardwareSkinning_Restore", 0.05, 100, function()
    local convar = GetConVar("r_forcehwskin")
    if convar and convar:GetString() == desiredForceHardwareSkinning then
        timer.Remove("RTXHardwareSkinning_Restore")
        if RTX_ReloadModelsAfterHardwareSkinningSettings then
            RTX_ReloadModelsAfterHardwareSkinningSettings()
        end
    elseif timer.RepsLeft("RTXHardwareSkinning_Restore") == 0 then
        ErrorNoHalt(string.format(
            "[gmRTX - Hardware Skinning] Restore failed: expected %s, got %s\n",
            desiredForceHardwareSkinning,
